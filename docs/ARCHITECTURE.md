...
src/
  ...
  svg/       body outline and muscle region paths (viewBox 0 0 260 620)
...
`svg/bodyPaths.ts` holds the outline and 24 muscle regions. `ui/MuscleMap.tsx` fills each region with a `color-mix()` of theme tokens: recovery uses negative → warning → positive, emphasis uses accent opacity, roles use accent tiers. Switching the theme re-colours the map with no extra code.
