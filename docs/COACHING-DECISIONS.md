...
- **Live heart line (owner's pick "E, layered trace" from the sample page):** `src/ui/PulseLine.tsx` draws a sharp trace over a blurred copy with a faint echo, fixed along the top of Train while the watch is LIVE; the top edge brightens on each beat and a heart + BPM number sits in the Train header (and replaces the dot in the live-session watch pill). Colour is the theme's `--accent` (re-read every second), so all five themes follow; reduced motion draws the trace once with no beat. Dev hook `__pulse(bpm)` fakes a LIVE reading for the gate.
