...
- **Strict-tool union limit (found live, 2026-09-23):** the API rejects a request whose strict tool schemas hold more than 16 union-typed parameters (type arrays or `anyOf`). `evaluate_plan`, `propose_program` and `propose_schedule` (7 nullable weekdays each) are no longer strict; the app already validates their inputs (`planDraftArg`, `buildAction`). The Worker schema test now caps unions at 12, and rejected requests carry the API's error text as `detail` (also logged for `wrangler tail`).
- **Strict grammar size (found live):** with 14 strict tools the API still refused ("compiled grammar is too large"). Strict is now limited to 8 small flat tools (`STRICT_TOOLS` in `tools/schema.ts`: goal, deload, start session, gym, snooze, escalate, remember, forget); the rest rely on the app-side validation in `actions.ts`, whose errors go back to the model as tool errors.
- **Apply for every proposal kind, pulled forward from EV6** after the owner could not apply a split. Each applier writes through the existing slice mutations (`createSplit`, `startSession`, `saveProfile`, `addGym`, profile setters…) and Undo restores exactly the state keys that kind touches, so anything done since stays. `propose_today` stores `escobar.todayOverride`; Train reads it in EV6.
- **Live heart line (owner's pick "E, layered trace" from the sample page):** `src/ui/PulseLine.tsx` draws a sharp trace over a blurred copy with a faint echo, fixed along the top of Train while the watch is LIVE; a heart + BPM number sits in the Train header (and replaces the dot in the live-session watch pill). Colour is the theme's `--accent` (re-read every second), so all five themes follow; reduced motion draws the trace once with no beat. Dev hook `__pulse(bpm)` fakes a LIVE reading for the gate.
- **No beating edge:** the owner kept the layered trace only; the brightening top edge was removed.
- **Heart pulse is "edge only" (F), final pick:** the owner clarified they meant F, not E. The trace is gone; the top edge brightens with each beat (theme `--accent`), with the heart + BPM number beating alongside.
- **Plan whiteboard (owner's pick, idea 3):** while a turn builds a plan (any of evaluate_plan / propose_program / propose_split, or two or more exercise searches), the live area shows a dashed "Building your plan" card instead of repeated "Searching exercises…" lines: one chip per search with its pattern and top result, then the draft days from the latest draft, then the volume check's verdict. Built only from the turn's stored tool calls and results (`ui/PlanBoard.tsx`). A running turn with nothing visible yet no longer draws an empty turn rule.
- **Live session reorder:** hold an exercise ~0.3 s and drag; the others slide aside and it drops in place (`slices/workout/reorder.ts`, `moveEntry` in `session.ts`). Movement only: no selection highlight, never starts from inputs or buttons, a move before the hold ends is a scroll, and the tap that ends a drag doesn't open/close the card. The open card follows its exercise.
- **Health Connect "Could not read" (owner report, 2026-09-23):** the plugin read `RestingHeartRateRecord` without ever declaring or requesting `READ_RESTING_HEART_RATE`; Health Connect throws `SecurityException` for a type without its read permission, and the plugin read all types in one try, so every sync failed even with every offered switch on (the sheet showed Vitals "0 of 1" — resting heart rate was never offered). Fix: the permission is declared (manifest patch, plugin alias, CI grep), each type is read on its own and only when granted (a refusal empties that type and is reported in `failed`), `needsPermission` means none granted, and the app asks once for newly missing permissions (`missing`). Source: developer.android.com Health Connect data types and permissions.
- **Escobar's mark is the owner's knot-and-plates logo** (2026-09-23), replacing the crown. It is extracted from the supplied PNG into a 192 px alpha mask (`src/assets/escobar-mark.png`) and drawn with `currentColor`, so it follows the nav's active/inactive colours and all five themes' accents; the knot's overlap outlines stay as gaps. While thinking, the whole mark breathes (static under reduced motion).

## Remediation R3: coach numbers (2026-09-23)
- **Body fat (BR-01):** the Navy equations take cm with their metric constants; the old inch conversion is gone. Tape inputs can be typed in cm or inches (inches by default for lb users) and are stored in cm.
- **Ready/full times count from now (BR-02),** and `readyInHours` is null once a muscle is ready.
- **One load ratio (BR-03, BR-19):** `acuteChronicRatio` feeds both the systemic recovery factor and readiness; it is null until 3+ sessions span 14 of the last 28 days. readiness() keeps only check-ins from the 30 days before today.
- **Active lifts (BR-05):** progress, effort and deload rules and the weekly review skip lifts not trained in 42 days.
- **Assisted work (BR-06):** less assistance is progress; with the assistance flat, best reps decide. e1RM-based rules skip non-weighted lifts.
- **Plateau lever (BR-04):** last 8 weeks, 6+ sessions, flat means under 1.5% total change over the window (slope × weeks). Six sessions is the evidence bar, so the trend's own "low confidence" (under 7 points) no longer blocks it. Priority 305, and only the highest-priority progress note per lift is shown (BR-27).
- **Volume status (BR-07):** judged on completed weeks: under after two weeks below the band, over when this or last week is above it. The copy says "last week".
- **One set count (BR-16):** `effectiveSetsByMuscle` (direct 1, secondary 0.5, stabiliser 0); "hard sets" leave out easy sets (they used to count half).
- **Balance (BR-17, QA-R3a-3/4):** counted per exercise set, split across its primary buckets. Upper is push **plus** pull (plan row 9), compared after dividing by `UPPER_PER_LOWER` = 1.5: two regions against one, so a balanced programme carries more upper sets. A full-body week (4 bench + 4 row + 4 squat, 2:1) and an even upper/lower split (24:24) both read balanced; the mean used before flagged the even split as "upper trailing". The ratio shown to the person is the plain set ratio.
- **D9:** above the band means smaller gains for more recovery cost, not harm; over-band weeks trigger a lighter-week offer only together with a stalled active main lift.
- **Pre-session (BR-08, BR-09, D10):** the brief quotes the same target as the set rows (readiness, deload, equipment). Warm-ups are 50/70/85% of the first working set, snapped to the equipment, and a step at or above the working load is dropped. Escobar's exercise card and next-target tool use the same base.
- **Effort calibration (BR-10, BR-11, D11):** the note now tells people how to rate ("Ideal is about 2, Easy 3 or more"); the bias is still not applied to e1RM. One observation per session pair, load and label.
- **HRmax (BR-12):** the highest qualifying plateau; a fresh observation never lowers the Tanaka estimate.
- **Weight trend (BR-14):** least-squares over the last 28 days (7+ weigh-ins spanning 14+ days), % of mean per week. **Pace** is only compared for rising lifts (BR-13). **Week grade (BR-22):** the planned days are the target (3 when nothing is planned).
- **Autoregulation without equipment (BR-18):** a step that rounds back to the target moves a full 2.5 kg.
- **Density (BR-20):** per main lift with 3+ sets, first vs last set; rest median without each lift's first set. **Records (BR-21, BR-28):** rounded, in the display unit, as typed. **Heart post notes (BR-26):** the session day and the next only. **Recovery driver (BR-31):** the biggest-dose set, with that exercise's own set count.
- **Plates (BR-24):** minimum-plate DP for the largest per-side sum at or under the target.
