...
| 2026-09-20 | Added scheduleDraft: the coach may now propose rearranging the weekly schedule (which split trains on which day), not just a split's own contents. Requested directly with three concrete scenarios: "switch my split Monday to upper", "switch [split] to Wed, and [split] to Friday", and an open "what do u think is best for my performance and body goal" — approve/decline, with reasoning, the same posture as a splitDraft. Extended the pattern end to end rather than special-casing it: `AskPayload` gained `schedule` (the real, current week — a genuine gap; the model previously had no way to see today's schedule to reason about it or preserve unstated days), `AskReply` gained `scheduleDraft` (rule 16, new): the FULL week every time, not a diff, mirroring the "modify" contract a splitDraft's exercises list already uses, since a partial per-day patch would need real merge logic the app doesn't have and the model can't be trusted to reason about correctly. Each day is a real split id from `splits` or null for rest; an unrecognized id or a missing day invalidates the *whole* scheduleDraft (one object, not an independent list like splitDrafts — a half-valid week isn't something to partially apply). Client: `src/ai/ask.ts`'s `parseScheduleDraft` re-validates against the payload's own known split ids (parse-time), `src/slices/workout/splits.ts`'s new `applyScheduleDraft` re-validates again against the live splits (apply-time) before the one-shot whole-record `update()` — same double-validation posture `applySplitDraft` already uses. The personal-scope number-check exemption (added for splitDrafts after the day-count bug) now also covers a reply with a scheduleDraft, for the same reason: explaining *why* a rearrangement makes sense ("training 5 days a week", "2 rest days between them") cites its own design numbers, not report facts. UI: `AskSheet.tsx`'s new `ScheduleDraftAction` shows only the days that actually differ from the live schedule right now (a `Mon: Rest` / `Wed: Push` style diff) rather than all 7 days, so the person can see exactly what they're accepting; applying calls `resyncReminders()` (schedule changes affect smart reminders) and `go('coach')` — Coach, not Train, since that's where the Weekly schedule section actually lives; a no-op when already there. Rule 16 explicitly tells the model not to propose a schedule change unless asked, or asked for a judgment — a schedule touches every day of the week, more disruptive than one split edit, so it doesn't invite itself into an unrelated answer. Verified live: seeded a real two-split schedule, asked to swap two days, got back the exact diff shown correctly, applied it, confirmed the resulting `state.schedule` matched exactly, zero console errors. Proxy and app both changed; needs a redeploy and a fresh build/Netlify deploy. |
| 2026-09-20 | Ran a persona-based answerability audit, requested directly: "pretend you are 100 users, different scenarios — new user, old-time user, medium user, kid, old age, teen, woman, men, etc. — then ask escobar different kind of questions... if those questions are not currently answerable, architect first and plan." Built a representative persona × question matrix across experience level, age/life-stage and gender, evaluated each against the exact live `promptAsk.ts` (no live API key in this sandbox, so this was reasoned analysis against the shipped rule text, not live model calls). Found six real gaps, all closeable at the prompt layer only — `AskPayload`/`GroundingPayload` never carries age, sex or pregnancy status (`validateGrounding`'s person-shaped-field blocklist in `handler.ts` rejects `profile`/`name`/etc. outright), so any persona context only ever exists if it's typed into `question` itself; there was nothing here for a new payload field to do. Four were wording gaps in rule 4/2: a stated child/young-teen age wasn't its own individualized case (an adult and a 13-year-old asking about supplement dosing got the identical answer); pregnancy/postpartum wasn't named anywhere, risking either an unwarranted refusal or an unwarranted specific answer; performance-enhancing drugs weren't named, leaving their framing entirely to base-model instinct; and a named-but-general chronic-condition question ("is cardio okay with asthma, in general") risked being over-deflected into the individualized path just for naming a condition, since rule 4's only worked example paired a condition with a genuinely individualized ask. Closed each with an explicit rule 4/2 clause and a matching general-answer test in `tests/ask-scenarios.test.ts`'s new persona-audit describe block. The other two — crisis/self-harm language and disordered-eating-adjacent requests — had no handling at all, and are architecturally different from a wording fix: this app's whole design principle is that a language model only writes, it never decides (see "Why this shape" above) — the same reasoning that keeps grounding facts deterministic says a safety-critical resource shouldn't exist only if the model remembers to mention it in free prose. So new rule 17 has the model set `concern` (`'crisis' | 'disordered_eating' | null`) as a genuine flag, the same shape as a finding or proposal, not prose — `AskSchema`/`AskReply`/`CallAsk` (`proxy/src/anthropic.ts`, `types.ts`) all gained it, `src/ai/ask.ts`'s `requestAskAnswer` validates it against the closed enum the same way `category` already is (anything else defaults to `null`, never trusted from the network), and `AskSheet.tsx` renders a small fixed, pre-written resource line (`ASK_CONCERN_RESOURCE`, pointing at findahelpline.com — an international directory, not a single country's hotline, since the app has no idea what country someone is in) whenever `concern` isn't null, regardless of what the model's own "answer" says. The model still writes its own warm, non-judgmental "answer" alongside the flag — rule 17 explicitly tells it never to go quiet or refuse to engage — but the resource itself is deterministic app content, the same "brain decides, words explain" split every finding/proposal already uses, just applied to a safety signal instead of a training one. Also raised `/ask`'s own reasoning effort from the shared `medium` (set earlier for latency) to a route-specific `ASK_EFFORT = 'high'` (`proxy/src/anthropic.ts`): the added rules are real judgment calls (stated-minor age vs. adult, general-vs-individualized for a named condition, whether a crisis signal is genuinely present), and effort is the lever that buys judgment depth without changing the schema or the timeout budget — the other five routes (explain, tag-exercise, notes, identify-exercise, import-programme) stay at `medium`, since none of their prompts gained this kind of nuanced call. Proxy and app both changed; needs a redeploy and a fresh build/Netlify deploy. |
| 2026-09-20 | Found live, direct report: the exercise library had essentially no functional training, calisthenics or home-workout content — a gap in the data, not the prompt or planners. Reviewed the whole 123-exercise `src/data/exercises.json` before adding anything (avoiding a near-duplicate of what was already there, e.g. `lib_goblet_squat` already covers kettlebell squats) and added 30 new exercises across three groups that were genuinely missing: functional/conditioning (burpee, mountain climbers, jumping jacks, high knees, jump rope, kettlebell swing, box jump, battle ropes, medicine ball slam, wall ball, bear crawl, renegade row), calisthenics strength (bodyweight squat, jump squat, bodyweight lunge, bodyweight split squat, pistol squat, wall sit, bodyweight calf raise, inverted row, pike push-up, diamond push-up, v-up, bicycle crunch, flutter kicks, hollow body hold, bird dog, superman), and light home-equipment (resistance band row, resistance band pull-apart). Every entry uses only existing muscle ids and movement patterns (`proxy/src/vocab.ts`'s `MUSCLE_IDS`/`PATTERNS`) — none of the new movements needed a new vocabulary value, since the existing patterns (`conditioning`, `carry`, `squat`, `lunge`, `single_leg_squat`, `hip_hinge`, `anti_rotation`, etc.) already cover this ground; only the entries themselves were missing. `src/core/exercises.ts`'s `inferMode()` reads equipment/name/a fixed id set to assign a `ResistanceMode`, so the genuinely explosive/metabolic-conditioning movements (burpee, mountain climbers, jumping jacks, high knees, jump rope, box jump, battle ropes, medicine ball slam, wall ball, bear crawl, jump squat) were added to `CONDITIONING_NAMES` and the two isometric holds (wall sit, hollow body hold) to `DURATION_NAMES` — everything else correctly falls out of the existing equipment-string check (`Bodyweight` → `bodyweight` mode) with no override needed. `proxy/src/vocab.ts`'s `EXERCISE_CATALOG` (the closed list `/ask`'s split-building draws from) was regenerated from the updated `exercises.json` so a splitDraft can actually propose these; `proxy/test/vocab.test.ts`'s drift check confirms the two copies still match exactly. Verified live: searched the exercise picker for six of the new names (burpee, wall sit, kettlebell swing, pistol squat, bird dog, resistance band row) and confirmed each is findable, added Burpee to a real split and confirmed it shows correctly tagged "Bodyweight, 3 sets" with working set-count controls, zero console errors. App-data-only; needs a fresh build/Netlify deploy, no proxy redeploy required for the app-side library itself — though the regenerated `EXERCISE_CATALOG` in `proxy/src/vocab.ts` does need one for `/ask` to be able to draft a split with these exercises. |
| 2026-09-20 | Found live, screenshots: every single `/ask` message ("Test", "Create dumbbell workout for me") failed with "schedule must have all 7 weekday keys, each a split id or null" — not a schedule question in sight. Root cause: `AskPayload.schedule` was made a hard-required field when scheduleDraft shipped (commit `abbc3bf`), and `validateAskPayload` 400s the whole request if it's missing, not just malformed. The proxy had already been redeployed with that requirement; the installed app build predates `abbc3bf` and has never sent `schedule` at all — a version-skew window inherent to this app's manual, two-step deploy (proxy and app/APK are redeployed separately, by hand, not atomically), and every `/ask` call from that gap breaks, not just schedule-related ones. Fixed the same way `preferences` already handles this exact class of problem: `schedule` is now optional on `AskPayload` (`types.ts`), `validateAskPayload` only rejects it when *present and malformed*, and a new `EMPTY_WEEK_SCHEDULE` constant (`types.ts`) is what `askMessages()` (`promptAsk.ts`) falls back to when it's absent — the model sees an honest "nothing scheduled" rather than the request being refused outright. An old client degrades to schedule-unaware answers (and can't get a scheduleDraft, which it couldn't render anyway) instead of every message failing. Did not extend the same treatment to `splits`, which has been stable since well before today and isn't the field actually breaking right now — narrow fix for the reported failure, not a preemptive rewrite of every payload field's strictness. The general lesson, worth remembering for the next payload field: a newly *required* field on a route the deployed app already calls is a breaking change the moment the proxy redeploys ahead of the app, so a genuinely new field should default to optional-with-a-safe-fallback unless there's a real reason the request is meaningless without it. Proxy-only; needs a redeploy — this one restores service to whatever app build people already have installed, without waiting on them to update it too. |
| 2026-09-20 | Found live, screenshots, right after the schedule-validation fix above shipped: a genuinely compound request — "My full body home, check current activities and suggest another workout for nxt days and scheduling" (review the current split, propose a new one, propose a schedule for it, all in one message) — 502'd with the same generic "The coach could not answer right now." Root cause: `callAsk` used `client.messages.parse(...)`, a buffered (non-streaming) call, with `max_tokens: 5000`. That number was tuned for a reply carrying prose plus several splitDrafts; today it also has to fit a scheduleDraft on top, *and* the just-raised `ASK_EFFORT = 'high'` (previous entry) spends noticeably more adaptive-thinking tokens before it even starts writing the answer — thinking tokens count against the same `max_tokens` ceiling on a non-streaming call. A compound turn like this one is exactly the shape most likely to hit that ceiling mid-JSON, the identical "cut short, `parsed_output` empty, `requireParsed` throws a 502" failure the day-count fix addressed earlier today, now reachable again by a different combination (schedule + higher effort) rather than the original one (day-count misread). Simply raising `max_tokens` again, on a non-streaming call, would just trade a truncation failure for an HTTP-timeout one on a big enough reply — the SDK's own guidance is to stream whenever a request may run long, precisely to avoid that tradeoff. Switched `callAsk` (only this route; the other five stay simple, low-token, non-streaming) to `client.messages.stream({...}).finalMessage()` — same params, same `parsed_output`/`stop_reason` shape `requireParsed` already expects, confirmed by a clean typecheck with zero other code changed — and raised `max_tokens` to 8000 and the client `timeout` to 120s now that a large reply is no longer at risk of blowing an HTTP timeout waiting to be buffered whole. Proxy-only; needs a redeploy. |
| 2026-09-20 | Found in the same screenshot as the 502 fix above, once the split it referenced was actually visible: "Full Body (Home)" had "Push-Up" listed three separate times (3, 3, then 2 sets) — a real splitDraft reply had repeated the same exerciseId within one entry's own "exercises" list, and nothing between the model and the screen ever caught it. `AskSchema`'s zod array type never enforced uniqueness, and `src/ai/ask.ts`'s `knownExercises()` — the client-side safety net that already drops an exercise id the app doesn't recognize — had no notion of "already kept this one" either, so three genuinely distinct entries sailed straight through into a real split. Fixed at the same safety-net function: `knownExercises()` now tracks a `Set` of exerciseIds already kept and silently drops a repeat, keeping the first occurrence's own "sets" (not a sum — a plain "same known id, second time, drop it" idiom, matching how it already drops an unrecognized id). Also added a line to rule 6 telling the model never to list the same exerciseId twice within one entry and to raise that exercise's own "sets" instead if it wants more volume on it — the prompt fix reduces how often this happens, the client fix is what actually guarantees it can't reach a real split either way. App-only; needs a fresh build/Netlify deploy. |
| 2026-09-20 | Reported live: "Create functional workouts for me 3" (a fresh user, zero splits logged) got the whole answer dropped — "The coach's answer used a number that is not in your data." Unlike the two fixes above, this one has no server logs to confirm the exact mechanism from, only the code path it's consistent with: the personal-scope number check runs whenever a reply has no surviving splitDraft (`requestAskAnswer`'s `drafts.length === 0` branch), so this means either the model asked a clarifying question but still tagged `scope: "personal"` with an example number in it, or it proposed drafts that didn't survive client-side validation. "...for me 3" is exactly the ambiguous shape rule 6's own vague-request bullet already exists to catch — a trailing bare number with no stated referent, genuinely unclear whether it means a day count, a request for 3 separate splits, or something else — so extended the existing day-count clause (rule 6) to name this shape explicitly and say to ask rather than guess, the same instinct that clause already applies to "5-day full body split." Lower confidence than the two fixes above — this narrows a real ambiguity rather than confirming a specific bug — worth re-checking against this exact phrasing once the current fixes are actually redeployed and it's reproducible live. Proxy-only; needs a redeploy. |
| 2026-09-20 | Feedback on a live example ("my age is 25, height 164cm, what's my bmi? what kg should I stick to, to get lean"): the answer opened with a full paragraph — the BMI formula, a healthy-range estimate from height alone, BMI's limitations, a pointer to the Body tab's tape-measure estimate — and only asked "what's your current weight?" in the last sentence. Technically correct (never invented a weight, did ask), but easy to miss under that much text, and not how a real coach would lead. This is a case rule 6 already has an answer for (splitDraft-building: "ask one or two short, specific questions first... do not guess a full plan from nothing") that general-knowledge answers never got an equivalent of. Added rule 18: when a genuinely useful, personalized number depends on a fact never in the report by design (the clearest case is current body weight — `validateGrounding` in `handler.ts` explicitly refuses a `bodyWeightKg` field outright, so it can only ever reach the model typed directly into the question) and the person hasn't stated it, lead with a short, direct question ("What's your current weight?") instead of opening with a population-average substitute and asking only as an afterthought — the same "ask before guessing" instinct rule 6 already uses. A genuinely separate part of the same question that doesn't need the missing figure (what BMI measures, the formula itself) can still get a full answer; the part that does need it should lead with the question, not bury it. Proxy-only; needs a redeploy. |
| 2026-09-20 | Generalized rule 18 immediately after, on direct request: "users want straight answers rather than long explanations... whatever the agent couldn't truthfully answer because of missing input or data, a one-line question should follow — a question first, then the user explains, then the response." The weight-specific wording above was really one instance of a broader standing behavior that only rules 6 and 16 had, each scoped to their own feature (splitDraft, scheduleDraft) — general-knowledge and personal answers had no equivalent. Rewrote rule 18 as the general case: whenever the coach genuinely cannot answer truthfully because something is missing (a fact it doesn't have and can't get from the report, or a request too vague to answer without guessing), lead with one short, direct question — not a paragraph of explanation with the question buried at the end or skipped for a hedge — the same instinct rules 6 and 16 already apply in their own domains, now named as the general rule they were both already instances of. A genuinely separate part of the same message that doesn't need the missing piece still gets a full, direct answer alongside the question. Proxy-only; needs a redeploy. |
| 2026-09-20 | Followed up: "implement that logic in whole thing of escobar" — directness shouldn't live only in rule 18's missing-info case, it should be how every reply reads. Rule 8 (answer length/shape) previously only capped word counts ("well under 120 words"... "up to about 160 words") without saying anything about *how* to use that budget — a reply could still open with a restated question or a caveat before ever answering, and stay technically under the cap. Rewrote its first sentence: lead with the direct answer, or with rule 18's one question, never a preamble, a restated version of the question, or a caveat stacked in front of the thing itself — and a shorter, direct answer beats a longer, more complete-sounding one whenever both would answer it. The existing length ceilings stay (a real UI constraint for a chat bubble, not re-litigated here); this changes what fills that space, not how much of it there is. The other rules already matched this instinct in their own domains (rule 1's "not vague hedging", rule 3's "say so plainly... rather than guessing or padding", rule 6/16's "ask... do not guess from nothing") — this is the one place a general style rule was missing it. Proxy-only; needs a redeploy. |
| 2026-09-20 | Caught a real risk in rule 18 as generalized above, before it shipped wrong: "Escobar's logic should also fall like urs [Claude's] — if an answer is answerable by research, then he should do that, instead of asking a probing question. Body weight or personal info should be probed, but if it's generalized then research will do." Rule 18's trigger ("something is missing") was broad enough to risk covering a general knowledge gap too — the coach not immediately knowing a fact it could work out or look up (rule 11's web search exists precisely for that) is not the same kind of "missing" as not knowing the person's own weight, but the wording didn't draw that line, so a future reply could have started asking the person to supply research instead of just doing it. Tightened the trigger itself to "something only the person can tell you" (their own body, history, preference or constraint), and added an explicit closing sentence: this is never a substitute for using what the coach already knows or reaching for web search when a question genuinely needs it — a general, researchable fact is not "missing" in the rule 18 sense, and asking the person to hand over research they're not equipped to do (that's the coach's job) is exactly backwards. Mirrors how Claude itself is expected to operate: probe the human for what only they know, look things up or reason through what's actually knowable. Proxy-only; needs a redeploy. |
| 2026-09-20 | Reported live, screenshots: weight and height were already set in Settings → Profile, but Escobar kept asking for them anyway — "what BMI should I stick to" got the same generic height-only range as before, never using the real numbers already sitting on the device. Root cause, confirmed against the code (not a bug in anything shipped today): `validateGrounding` in `handler.ts` has always refused a request outright if `bodyWeightKg` or `heightCm` appears anywhere in it — a deliberate, hard privacy boundary, not an oversight, so raw weight/height were never going to reach the proxy no matter what Settings held. Put the actual tradeoff to the user directly rather than picking one myself, since it's a real privacy decision: keep the boundary as-is (Escobar keeps asking in chat), add a local-only BMI display (pure on-device math, nothing sent anywhere), or let Escobar see a *derived* BMI number instead of raw weight/height. Chosen: the derived-number option. Added `computeBmi()` (`src/brain/coach/explainer.ts`) — BMI = kg / m², one decimal, null when either figure isn't set, never a guess — and a new `bmi?: number | null` field on the shared `GroundingPayload` (both the proxy's `types.ts` copy and the app's `explainer.ts` copy), populated only by `/ask`'s `buildAskPayload` (via a new `AskSheet.tsx` call to `computeBmi(state.profile)`) since that's the one interactive route that needs it — `/explain`'s builder leaves it undefined, harmless since it's optional. `validateGrounding` gained one narrow exception: `bmi`, when present, must be a plausible positive number (bounded, same "don't trust the network" discipline as every other field) — `bodyWeightKg`/`heightCm` themselves stay exactly as refused as before; a payload can carry the ratio, never the raw figures it came from. `allowedNumbers()` (`explainer.ts`) extended so a personal-scope answer stating the real BMI number isn't rejected as an invented one. `promptAsk.ts` rule 18 gained the one exception to "their current weight is never in the report": when `bmi` is present, state it directly and confidently, never ask for it — but weight and height themselves still never reach the model, so a question that needs one specifically (converting a target BMI back into a kg number) still asks, now just for height alone rather than both figures. Verified live: set weight 70kg/height 164cm in Settings, asked "what bmi should i stick, what kg too for lean," confirmed the real request payload carried `bmi: 26` and no trace of `bodyWeight`/`heightCm` anywhere in it, and a reply correctly citing "26" rendered without the personal-scope number check rejecting it. Both sides changed; needs a redeploy and a fresh build/Netlify deploy. |
| 2026-09-20 | Reported live, two connected failures on a new user's first real attempt at rearranging their weekly schedule. (1) Asked "change my ppl to tths" (Tue/Thu/Sat) — Escobar's "answer" said "done, changed to tue thurs sat," but Weekly schedule still read Mon/Wed/Fri afterward. High confidence on this one: applying a scheduleDraft (or a splitDraft) has always needed an explicit tap on the action button the app shows under it — nothing in `promptAsk.ts` ever told the model that, so it wrote its proposal in the past tense as if the tap had already happened. A real, if unintentional, false claim about the person's own data — exactly the class of thing this app's whole design is built to prevent. Added a bullet to rule 6 (and the same idea to rule 16) telling the model to write "answer" as a proposal still waiting on them ("I'd move Push to Tuesday...") and never as something already done ("I've changed it," "done"). (2) Retried, Escobar asked a clarifying question (which days, in what order), the person answered "yes," and got the personal-scope number-check rejection instead of an actual scheduleDraft. Lower confidence on the exact mechanism — no logs, only the code path it's consistent with: a scheduleDraft that failed `parseScheduleDraft`'s validation falls back to running the number check against "answer," so either the model didn't build a draft in that reply at all, or built one that didn't survive. Closed the gap that's clearly real either way: rule 16 never explicitly said what rule 6 already does ("once you have enough, propose it in the same reply... not an extra turn later") — added the equivalent for scheduleDraft: once their next message is their answer to your own clarifying question, even a bare "yes," build the actual scheduleDraft in that same reply rather than asking again or deferring it. Proxy-only; needs a redeploy. |
