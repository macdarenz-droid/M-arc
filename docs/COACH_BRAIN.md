...
- **The one deliberate exception: `/build-split`.** "The language model
  only writes; it never decides" above still holds for the report,
  findings and every proposal Layer 1 produces — and for the general "Ask
  the coach" chat, whose own prompt explicitly declines to build a plan.
  `/build-split` (its own proxy route, its own entry point on the Train
  tab, see the 2026-09-20 decision log entry below) is the one place a
