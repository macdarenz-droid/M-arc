...
**Status, 2026-09-21:** F0, rack swap (1), effort-graded rest (2), morning verdict (3), session-plan capture (10A/10AT), live autoregulation (4A-4T), warm-up ramp (5A-5T), chronic skip (6A-6T), closed-week review (7A-7T), lift trajectory (8A-8T), and unfinished-item work items 9A-9B are implemented and committed unit by unit on the working branch. Feature 10's debrief calculation/UI remains later in the routed order.
...
Mount the bounded Unfinished list before suggestions, open saved turns in AskSheet without network access, and revalidate each pending key before apply/dismiss. Show deterministic local reappearance copy and pass the freshly computed report into explicit proposal dismissals. Exact scope and next feature order are in [02-ROUTING-PLAN.md](02-ROUTING-PLAN.md).
