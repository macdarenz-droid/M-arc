...
**Status, 2026-09-21:** F0, rack swap (1), effort-graded rest (2), morning verdict (3), session-plan capture (10A/10AT), live autoregulation (4A-4T), warm-up ramp (5A-5T), chronic skip (6A-6T), closed-week review (7A-7T), lift trajectory (8A-8T), and unfinished-item work items 9A-9C are implemented and committed unit by unit on the working branch. Feature 10's debrief calculation/UI remains later in the routed order.
...
Run the full app/proxy gate and feature-specific export/restore plus reopened accept/dismiss checks. Inspect the five-theme screenshots, append the dated decision entry, commit the feature gate, mirror it to GitHub and check status for the exact commit. Exact scope and next feature order are in [02-ROUTING-PLAN.md](02-ROUTING-PLAN.md).
