...
- Addressed Claude's five QA findings on `7f36209`: optional native reads render alongside the shell and fail with a notice when no handover is known; confirmed/prepared handovers protect only the live workout while history, settings and exports remain usable; early crash reset checks the stored marker only; web storage read failures cannot create a watch lock; browser assertions use both actual reset labels. Added focused ownership, real inline crash-reset and browser navigation/export/timeout regressions. Local checks pass: 26 focused tests, all 911 unit tests, 3 performance budgets, TypeScript, production build and agent guard. CI browser/Android results are pending; local Chromium download was an invalid archive. No main merge or watch activation.
