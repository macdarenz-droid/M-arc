...
describe('one report on ninety sessions stays cheap', () => {
  it('builds in well under a second', () => {
    ...
    const ctx = { sessions: h.sessions, splits: h.splits, schedule: h.schedule, custom: [], goal: h.goal, restDefaultSec: 90, health: { connected: false }, readiness: [], deload: null, today: h.today, now: d.getTime(), dismissed: {}, accepted: {} };
