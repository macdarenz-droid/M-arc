...
import { heartRateContext, HEART_RATE_EVIDENCE } from '@/brain/heart-rate';
import { Button, Card, Chip, Section } from '@/ui/primitives';
import { formatDay } from '@/core/dates';
import { contextAdvice, contextObservation, contextReason, STATE_LABEL } from './words';
...
export function WatchInsights({ compact = false }: { compact?: boolean }) {
  ...
  // A selected past workout is evaluated with only the evidence that existed
  // when it ended — never with workouts that came after it.
  ...
  return (
    <Section title="Watch insights" aside={compact
      ...
      : <Chip tone={context.state === 'ready' ? 'info' : undefined}>{STATE_LABEL[context.state]}</Chip>}>
      <Card class="watch-insights" data-testid="watch-insights" data-state={context.state}>
        ...
        <h3 style={{ margin: '6px 0' }}>{observation || contextReason(context)}</h3>
        <p class="small muted">{contextAdvice(context)}</p>
        {comparison && (
        ...
        {!compact && <>
          <p class="hint" style={{ marginTop: 12 }}>{contextReason(context)}</p>
          ...
          <details class="watch-method">
            ...
            <p>Escobar compares the same exercises and loads with similar volume and duration, using at least {HEART_RATE_EVIDENCE.minimumBaselineSessions} earlier recordings from the past six weeks. It checks signal coverage before comparing, and an accepted easier week is only ever compared with other easier weeks.</p>
            ...
            <p>Capture coverage is the share of elapsed workout time represented by usable readings, allowing at most five seconds per reading. Comparisons need {HEART_RATE_EVIDENCE.minimumCoveragePct}% coverage and three captured minutes. These are data-quality rules, not health thresholds. Pauses remain part of the recording.</p>
            <p>Load progression, rest timers and muscle recovery still use your logged performance and effort. Watch readings do not automatically change weights, rest timers, calories, or training zones.</p>
          </details>
        </>}
      </Card>
    </Section>
  ...
  return (
    <svg class="watch-history-chart" viewBox="0 0 336 154" role="img" aria-label="Average heart rate by recent workout; use the session buttons below for details">
      ...
      {points.map((p, i) => p.averageBpm == null ? null : <g key={p.sessionId} onClick={() => onSelect(p.sessionId)}>
        ...
        <title>{p.day}: {p.averageBpm} BPM; {p.coveragePct ?? 'unknown'}% coverage.</title>
      </g>)}
