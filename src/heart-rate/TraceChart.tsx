...
export function TraceChart({ samples, session }: { samples: HeartRateSample[]; session: Session }) {
  ...
  // Logging marks, not set boundaries: they say when a row was committed.
  ...
  return (
    <div class="watch-trace" onClick={e => e.stopPropagation()}>
      ...
      <p class="hint" aria-live="polite">{minute(picked.receivedAtEpochMs)} min · {picked.bpm} BPM{logged.length ? ' · Ticks show when sets were logged, not when work started or stopped.' : ''}</p>
    </div>
  );
}
