/**
 * The measurement contract, version 2. The same calculation exists in
 * native Java (HeartRateMetrics.java) with matching scenarios; the two must
 * not disagree.
 *
 * Signal completeness is not physiological accuracy. None of these rules
 * establish that a reading is correct — only that it was captured, in
 * bounds, and inside the workout window.
 */
...
export function summarize(samples: HeartRateSample[], startedAt: number, endedAt: number, gapCount = 0): HeartRateSummary {
  ...
  // Every accepted reading covers at least 1 ms, so capturedMs cannot be zero
  // here. Guarding anyway keeps a future bound change from emitting NaN BPM.
  if (!accepted.length || capturedMs <= 0) return base;
  return {
    ...base,
