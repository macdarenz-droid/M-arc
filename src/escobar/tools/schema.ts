...
export const TOOLS: ToolDef[] = [
  ...
  T({ name: 'evaluate_plan', kind: 'read', strict: true, status: 'Checking the plan against your volume and recovery…', description: 'Grade a programme draft: weekly sets per muscle against this person’s bands, balance, back-to-back hard days, rep ranges and session length, with blocking issues. Call before propose_program and revise until no issue has severity "block".', input_schema: obj({ draft: PLAN_DRAFT }, ['draft']) }),
  ...
  T({ name: 'propose_split', kind: 'act', strict: true, status: 'Drafting the split…', description: 'Propose creating, changing or deleting one split. The person sees a card with the changes and taps Apply. Use exercise ids from search_exercises.', input_schema: obj({ action: en(['create', 'modify', 'delete']), splitId: str('Required for modify and delete.'), name: str('At most 28 characters.'), focus: arr(MUSCLE, 'Up to 2 muscles to bring up.'), exercises: arr(obj({ exerciseId: EXERCISE_ID, sets: int('1–6.') }, ['exerciseId', 'sets']), 'At most 14.') }, ['action', 'name', 'exercises']) }),
  T({ name: 'propose_program', kind: 'act', strict: true, status: 'Drafting the programme…', description: 'Propose a whole programme (splits plus weekly schedule). Only after evaluate_plan shows no blocking issues. replaceExisting swaps out the current splits.', input_schema: obj({ draft: PLAN_DRAFT, replaceExisting: { type: 'boolean' } }, ['draft', 'replaceExisting']) }),
  T({ name: 'propose_schedule', kind: 'act', strict: true, status: 'Drafting the schedule…', description: 'Propose a new weekly schedule: a split id or null (rest) for each day.', input_schema: obj({ week: obj(Object.fromEntries(WEEKDAYS.map(d => [d, { type: ['string', 'null'] }])), [...WEEKDAYS]) }, ['week']) }),
  T({ name: 'propose_goal', kind: 'act', strict: true, status: 'Preparing the goal change…', description: 'Propose switching the training goal. Changes rep ranges, not exercises.', input_schema: obj({ goal: en(GOALS.map(g => g.id)), applyGoalRest: bool('Also set the goal’s default rest.') }, ['goal']) }),
  T({ name: 'propose_today', kind: 'act', strict: true, status: "Adjusting today's session…", description: "Propose changes to today's session only (swap, remove, add, sets, or a load factor), e.g. around soreness or time. Never changes the saved split.", input_schema: obj({ splitId: str('Today’s scheduled split, or the one they will train.'), changes: arr(TODAY_CHANGE), reason: str('One line the person will see.') }, ['splitId', 'changes', 'reason']) }),
  T({ name: 'propose_deload', kind: 'act', strict: true, status: 'Preparing a lighter week…', description: 'Propose a 7-day lighter week (fewer sets, lighter loads).', input_schema: obj({ reason: str('One line.') }, ['reason']) }),
  T({ name: 'propose_start_session', kind: 'act', strict: true, status: 'Getting the session ready…', description: 'Offer to start a session with a split now.', input_schema: obj({ splitId: str() }, ['splitId']) }),
  T({ name: 'propose_checkin', kind: 'act', strict: true, status: 'Preparing a check-in…', description: "Propose saving today's check-in from what the person told you (sleep quality, mood, soreness 1–5).", input_schema: obj({ sleepQuality: RATING, mood: RATING, soreness: arr(obj({ muscle: MUSCLE, level: RATING }, ['muscle', 'level'])) }) }),
  T({ name: 'propose_profile', kind: 'act', strict: true, status: 'Preparing a profile update…', description: 'Propose updating one profile field the person stated (weight in kg, height in cm, birth year, sex, training since YYYY-MM, planned days per week).', input_schema: obj({ field: en(['bodyWeightKg', 'heightCm', 'birthYear', 'sex', 'trainingSince', 'plannedDays']), value: { type: ['string', 'number'] } }, ['field', 'value']) }),
  T({ name: 'propose_custom_exercise', kind: 'act', strict: true, status: 'Drafting a custom exercise…', description: 'Propose adding an exercise that is not in the library.', input_schema: obj({ name: str('At most 60 characters.'), equipment: str(), primary: arr(MUSCLE, '1–2 muscles.'), secondary: arr(MUSCLE), mode: en(['weighted', 'bodyweight', 'assisted', 'duration', 'conditioning']), role: en(['main', 'accessory']) }, ['name', 'equipment', 'primary', 'secondary', 'mode', 'role']) }),
  T({ name: 'propose_reminder', kind: 'act', strict: true, status: 'Preparing reminder settings…', description: 'Propose training-day reminder settings (on/off, time, style, readiness summary) when the person asks for reminders.', input_schema: obj({ enabled: { type: 'boolean' }, time: str('HH:MM, 24-hour.'), style: en(['silent', 'vibrate', 'alert']), readinessSummary: { type: 'boolean' } }, ['enabled']) }),
  T({ name: 'propose_setting', kind: 'act', strict: true, status: 'Preparing a settings change…', description: 'Propose changing one app setting: rest mode, automatic rest, rest length, display unit, the daily quote or haptics.', input_schema: obj({ setting: SETTING }, ['setting']) }),
  T({ name: 'snooze_insight', kind: 'act', strict: true, status: 'Noting that…', description: "Mark an insight helpful or snooze it for 7 days, when the person says so. Applied at once with Undo.", input_schema: obj({ insightId: str('From get_insights.'), verdict: en(['snoozed', 'helpful']) }, ['insightId', 'verdict']) }),
  T({ name: 'propose_equipment_profile', kind: 'act', strict: true, status: 'Preparing the equipment profile…', description: 'Propose how a piece of equipment loads at a gym (unit, steps, dumbbell ladder, bar, plates), e.g. from a rack photo or "the dumbbells here are in pounds". If unsure of the unit, ask one question first.', input_schema: obj({ scope: en(['exercise', 'equipment']), exerciseId: EXERCISE_ID, equipmentGroup: en(['Barbell', 'Dumbbells', 'Machine', 'Cable', 'Smith Machine', 'Bodyweight', 'Sled', 'General']), gymId: str('Default: the active gym.'), profile: EQUIPMENT_PROFILE }, ['scope', 'profile']) }),
  T({ name: 'propose_gym', kind: 'act', strict: true, status: 'Preparing a new gym…', description: 'Propose adding a gym and making it the active one.', input_schema: obj({ name: str('At most 28 characters.'), defaultUnit: en(['kg', 'lb']) }, ['name', 'defaultUnit']) }),
  T({ name: 'escalate', kind: 'act', strict: true, status: 'Adding something important…', description: 'Show the app’s fixed safety card: pain (sharp, spreading, numb, or lasting past 48 h), medical (chest pain, fainting, dizziness), crisis, or disordered eating. Always use it in those situations.', input_schema: obj({ kind: en(['pain', 'medical', 'crisis', 'disordered_eating']), note: str('Optional, one line.') }, ['kind']) }),

  // Memory
  T({ name: 'remember', kind: 'memory', strict: true, status: 'Remembering that…', description: 'Remember a durable fact the person stated (injury, equipment, preference, goal in their words, agreement). Written at once with Undo. Not for feelings or one-offs.', input_schema: obj({ kind: en(MEMORY_KINDS.filter(k => k !== 'episode')), text: str('At most 200 characters, plain words.'), expiresInDays: int('Injuries default to 42.') }, ['kind', 'text']) }),
  T({ name: 'forget', kind: 'memory', strict: true, status: 'Forgetting that…', description: 'Forget one memory item when the person asks.', input_schema: obj({ memoryId: str() }, ['memoryId']) }),
  T({ name: 'recall', kind: 'memory', strict: true, status: 'Checking what I know…', description: 'Look up remembered items beyond the 12 in the brief.', input_schema: obj({ kind: en(MEMORY_KINDS), query: str() }) }),
];
