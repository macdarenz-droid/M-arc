...
import { askTurnFingerprint, pendingCoachItems, type PendingCoachItem } from '@/brain/coach/askDrafts';
import { currentGrounding } from './escobarContext';
...
export function AskSheet({ onClose, initialTurnKey, savedOnly = false }: { onClose: () => void; initialTurnKey?: string; savedOnly?: boolean }) {
  ...
  const send = async () => {
    ...
    // The whole brain's current read (brain/coach/escobar.ts): every rule, readiness, recovery,
    // volume, records and today's targets. Stated constraints ride along as preferences.
    const payload = buildAskPayload(currentGrounding(), plainHistory, q, { splits: s.splits, customExercises: s.customExercises, schedule: s.schedule });
  ...
  return (
    <Sheet title={`Ask ${COACH_NAME}`} onClose={onClose}>
      <div class="ask-thread" ref={threadRef}>
        {!history.length && <p class="small muted">Ask anything — your own training, general questions about exercise, muscles or nutrition, describe a split to build or change, or ask about rearranging your weekly schedule. Personal answers read the same coach you see in the app: readiness, recovery, volume, records and today's targets. Nothing changes until you tap an action.</p>}
