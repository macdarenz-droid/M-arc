...
1. In the current Huawei-supported DevEco toolchain, create a **new [Lite] Empty Ability** project with device type **Lite Wearable**, compatible with the actual GT6 firmware. Huawei's [Lite Wearable development guide](https://developer.huawei.com/consumer/cn/doc/best-practices/bpta-lite-wearable-guide) lists WATCH GT 6 at 466 × 466 and names this template. Proposed probe bundle: `com.mrcdrnzz.watchlab`. If that target is unavailable or cannot be installed on this regional model, record the exact error: this is a Gate A blocker, not a reason to silently substitute a full HarmonyOS/ArkTS project.
...
Huawei's [JS P2P reference](https://developer.huawei.com/consumer/en/doc/connectivity-References/point-to-point-message-0000001052164296) notes that some Lite Wearable versions do not expose `setPeerFingerPrint` and use template configuration for the peer fingerprint. The probe checks for that method rather than crashing. If absent, verify the official template's peer identity configuration against the installed SDK documentation; do not invent a config key or mark messaging passed from a receiver callback alone.
