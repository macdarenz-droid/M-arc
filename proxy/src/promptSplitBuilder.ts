 ...
 * that only designs or adjusts splits (reusable workout templates), one or
 * several per turn, through a short back-and-forth. Cached like every
 * other route's prompt.
 *
...
2. "splitDrafts" is a list, not a single slot — put one entry in it per split you are actually proposing this turn. Most turns propose exactly one, but when the person describes several splits at once (a full weekly plan, "Push" and "Pull" both pasted in one message), put one entry per split in the same reply rather than picking just one and ignoring the rest or spreading them across several turns. Leave it an empty list and just talk in "answer" when nothing is concrete yet. Each entry's "exercises" must use only ids from the catalog above, 3 to 8 of them, each with a sensible set count for the goal (usually 2-5, never above 6). Each entry's "focus" is at most 2 muscle ids — the main thing that split is for, matching the app's own "up to two muscles you want to bring up" idea, not every muscle the exercises happen to touch.
3. Use "action": "modify" and the matching splitId only when the person is clearly talking about one of their existing splits listed below (they name it, or it's the obvious one from context) — and in that case, that entry's "exercises" must be the split's FULL exercise list after your change (everything that should stay, plus whatever's new), not just the new part, since the app replaces the list with exactly what you send. Use "action": "create" with splitId null for anything that isn't clearly an edit to an existing split.
...
Return JSON matching the schema: "answer" and "splitDrafts" (a list, possibly empty).`;
