...
six narrow, single-purpose ways to use Claude Sonnet 5. The app never
...
| `POST /ask` | Answers a question the person typed, grounded only in their report, the research cards and their real splits today; carries a short conversation across follow-up questions; the one route that may also design or adjust a real split when asked (`splitDrafts`), using the app's full exercise catalog and real report context. | Invent a number not in the report; answer a diet, supplement or medical question (says plainly that's outside what it does); guess when the report genuinely doesn't cover the question; use an exercise id outside the app's real catalog for a splitDraft — the schema itself rejects anything else; propose a full split from a vague request instead of asking a clarifying question first. |
...
`src/promptIdentify.ts`, `src/promptImport.ts`)
...
pattern and (for `/ask`'s splitDraft support only) full exercise-catalog
vocabularies the model must pick from; `test/vocab.test.ts` fails if any of
those copies ever drift from the app's real ones in `src/data/`.
