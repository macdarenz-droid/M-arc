...
five narrow, single-purpose ways to use Claude Sonnet 5. The app never
...
`src/promptIdentify.ts`) and its own structured-output schema, so a
malformed reply is rejected before the app ever sees it. `src/vocab.ts`
holds the muscle and movement-pattern vocabularies the model must pick
from; `test/vocab.test.ts` fails if that
copy ever drifts from the app's real ones in `src/data/`.
...
smaller requests and cost less each. `/identify-exercise` costs noticeably
more than the others — a photo's image tokens dwarf a short text payload —
but it is a one-off, deliberate action (saving one custom exercise), not
something called on every screen. The Worker itself runs inside
