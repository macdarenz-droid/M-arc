...
Escobar proactive-coach implementation has started from documentation commit `e694f004aeb9eb38e6ba30bcaf080b942d75cce3`. F0, rack swap (1A–1T), effort-graded rest (2A–2T), morning verdict (3A–3T), the session-plan capture prerequisite (10A/10AT), live autoregulation (4A–4T), warm-up ramp (5A–5T), chronic skip (6A–6T), and closed-week review (7A–7T) are complete in this checkout. Closed-week comparison excludes the current partial week and first partial observation week, retains observed zero weeks and renders offline from a narrowly invalidated computed signal. Claude Opus 5's ranking and architecture remain the contract.

Read `docs/escobar-roadmap/HANDOFF.md` for the spec index and `docs/escobar-roadmap/02-ROUTING-PLAN.md` for dependencies/model routing. Continue with **8A**, the pure lift-trajectory module and arithmetic tests. Preserve the anchored projection and expiry semantics; never render the relative trend slope as kg/week. Follow `MODEL_ROUTER.md`, copied unchanged from the owner's supplied protocol.
